...
class MessageService {
    async initialize() {
        GuildManager = (await import('../models/GuildManager')).default;
        CommandHandler = (await import('../models/CommandHandler')).default;
        IntegrationManager = (await import('../models/IntegrationManager')).default;
    }
    
    process = async function(message, external = false) {
    ...
    sendMessage = function(string, target) {
        return target.send(string, { split: true });
    }
    
    sendCommandError = function(command, alias, target) {
        if (command) {
            return target.send(`Usage: ${_utils.cmdHelp(command, alias)}`, { split: true });
        }
    }
    
    messageChannelById = function(string, guild, id) {
